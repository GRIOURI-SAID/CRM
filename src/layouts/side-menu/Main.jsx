...
function Main() {
  ...
  const sideMenu = () => nestedMenu($h.toRaw(currentMenuArr), location);
  const [currentMenuArr, setCurrentMenuArr] = useState([]);

  useEffect(() => {

    const mainFetch = async () => {
      const res = await getRouterPermissions()
      console.log("res", res);
      const baseMenu = sideMenuStore.menu
      console.log("baseMenu", baseMenu);
      const menuTemp = []
      baseMenu.forEach((menu) => {
        res.forEach((result) => {
          if (menu.permission.toLowerCase().includes(result.module.toLowerCase())) {
            menuTemp.push(menu)
          }
        })
      })
      setCurrentMenuArr(menuTemp)
    }
    mainFetch();
  ...
  useEffect(() => {
    ...
    const currentMenuTemp = sideMenu().filter(menu => menu.active == true);
    setCurrentMenu(currentMenuTemp.length > 0 ? currentMenuTemp[0].title : null);
  }, [sideMenuStore, location.pathname, currentMenuArr]);
    ...
    <div className="py-2">
      ...
      <div className="flex mt-[4.7rem] md:mt-0">
        ...
        <div className="content">
          <TopBar currentMenu={currentMenu} />
          <Outlet />
        </div>
        {/* END: Content */}
      </div>
